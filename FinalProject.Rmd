...
A quick classification tree (CART) model, to create an initial analysis of the possible model
```{r cart, message=FALSE}
library(caret)
modelFile <- "rfModel.rda"
if (!file.exists(modelFile)) {
        trControl = trainControl(method = "cv", number = 5, allowParallel = TRUE)
        modelFit <- train(classe ~ ., data = training[, c("classe", relevantFeatures)], method = "rf", trControl = trControl)
